...
public class ConsumptionCertificate_ROSEMOUNT extends Certificate {
    ...
    public void init(Calculation result, HashMap<Integer, Object> values, Channel channel) {
        ...
        if (this.result.goodChannel()){
            if (os == SystemData.SYS_UNIX){
                formFile = FileBrowser.LINUX_FILE_CONSUMPTION_ROSEMOUNT_GOOD;
            }else {
                formFile = FileBrowser.FILE_CONSUMPTION_ROSEMOUNT_GOOD;
            }
        }else{
            this.numberOfReference = (String) values.get(Key.CHANNEL_REFERENCE);
            this.badMessage = (String) values.get(Key.CHANNEL_IS_GOOD);
            if (os == SystemData.SYS_UNIX){
                formFile = FileBrowser.LINUX_FILE_CONSUMPTION_ROSEMOUNT_BAD;
            }else {
                formFile = FileBrowser.FILE_CONSUMPTION_ROSEMOUNT_BAD;
            }
        }
        try{
    ...
    public void putResult() {
        ...
        cell(31, 2).setCellValue(VariableConverter.roundingDouble2(values[0], Locale.GERMAN));
        cell(33, 2).setCellValue(VariableConverter.roundingDouble2(values[1], Locale.GERMAN));
        cell(35, 2).setCellValue(VariableConverter.roundingDouble2(values[2], Locale.GERMAN));
        cell(38, 2).setCellValue(VariableConverter.roundingDouble2(values[3], Locale.GERMAN));
        ...
        if (this.result.getSystematicErrors()[1] < 0.01 && this.result.getSystematicErrors()[1] > -0.01){
            s91 = VariableConverter.roundingDouble3(this.result.getSystematicErrors()[1], Locale.GERMAN);
        }else {
            s91 = VariableConverter.roundingDouble2(this.result.getSystematicErrors()[1], Locale.GERMAN);
        }
        if (this.result.getSystematicErrors()[2] < 0.01 && this.result.getSystematicErrors()[2] > -0.01){
            s305 = VariableConverter.roundingDouble3(this.result.getSystematicErrors()[2], Locale.GERMAN);
        }else {
            s305 = VariableConverter.roundingDouble2(this.result.getSystematicErrors()[2], Locale.GERMAN);
        }
        if (this.result.getSystematicErrors()[3] < 0.01 && this.result.getSystematicErrors()[3] > -0.01){
            s914 = VariableConverter.roundingDouble3(this.result.getSystematicErrors()[3], Locale.GERMAN);
        }else {
