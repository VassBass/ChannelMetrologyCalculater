...
public class ConsumptionCertificate extends Certificate {
    ...
    public void init(Calculation result, HashMap<Integer, Object> values, Channel channel) {
        ...
        if (this.result.goodChannel()){
            try{
                POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(FileBrowser.FILE_CONSUMPTION_GOOD));
            ...
            try{
                POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(FileBrowser.FILE_CONSUMPTION_BAD));
                this.book = new HSSFWorkbook(fs);
            }catch (Exception ex){
                ex.printStackTrace();
            }
    ...
    public void putChannelData() {
        ...
        cell(33, 15).setCellValue(this.measurementValue);
    ...
    public void putResult() {
        ...
        cell(30,13).setCellValue(s25);
        cell(31,13).setCellValue(s50);
        cell(32,13).setCellValue(s75);
        cell(33,13).setCellValue(s100);
