    ...
    public double[][] getErrorsAbsolute() {
        if (this.errorsAbsolute == null) {
            switch (this.channel.getMeasurement().getNameConstant()) {
                ...
                case CONSUMPTION:
                    if (this.calibrator.getName().equals(Strings.CALIBRATOR_ROSEMOUNT_8714DQ4)){
            ...
            switch (this.channel.getMeasurement().getNameConstant()){
                ...
                case CONSUMPTION:
                    ...
                    if (this.calibrator.getName().equals(Strings.CALIBRATOR_ROSEMOUNT_8714DQ4)){
                        sumC = new double[4];
                        for (double[] doubles : errorsAbsolute) {
                            sumC[0] += Math.pow(doubles[0], 2D) + Math.pow(doubles[1], 2D);
                            sumC[1] += Math.pow(doubles[2], 2D) + Math.pow(doubles[3], 2D);
                            sumC[2] += Math.pow(doubles[4], 2D) + Math.pow(doubles[5], 2D);
                            sumC[3] += Math.pow(doubles[6], 2D) + Math.pow(doubles[7], 2D);
                        }
                        sumC[0] = Math.sqrt(sumC[0]/20);
                        sumC[1] = Math.sqrt(sumC[1]/20);
                        sumC[2] = Math.sqrt(sumC[2]/20);
                        sumC[3] = Math.sqrt(sumC[3]/20);
                        ...
                        for (double[] doubles : errorsAbsolute) {
                            ...
                            sumC[2] += Math.pow(doubles[4], 2D) + Math.pow(doubles[5], 2D);
                            sumC[3] += Math.pow(doubles[6], 2D) + Math.pow(doubles[7], 2D);
                            sumC[4] += Math.pow(doubles[8], 2D) + Math.pow(doubles[9], 2D);
                        }
                        sumC[0] = Math.sqrt(sumC[0] / 20);
                        sumC[1] = Math.sqrt(sumC[1] / 20);
                        sumC[2] = Math.sqrt(sumC[2] / 20);
                        sumC[3] = Math.sqrt(sumC[3] / 20);
                        sumC[4] = Math.sqrt(sumC[4] / 20);

                    }
                    this.uA = NumberUtils.max(sumC);
                    return this.uA;
                default: return 0D;
            }
        }else {
