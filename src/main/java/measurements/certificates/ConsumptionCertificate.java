...
public class ConsumptionCertificate implements Certificate {
    ...
    public void init(Calculation result, Values values, Channel channel) {
        ...
        if (this.result.goodChannel()){
            try{
                POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(Files.FILE_FORM_CONSUMPTION_GOOD));
            ...
            try{
                POIFSFileSystem fs = new POIFSFileSystem(new FileInputStream(Files.FILE_FORM_CONSUMPTION_BAD));
                this.book = new HSSFWorkbook(fs);
            }catch (Exception ex){
                ex.printStackTrace();
            }
